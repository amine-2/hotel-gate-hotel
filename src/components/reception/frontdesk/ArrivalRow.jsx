      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
            {booking.channel === "walk_in" ? "On-site" : "Online"}
          </span>
        </div>
        ...
        <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <span>{booking.room?.room_type?.name?.en || "Room"}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {stay?.status === "reserved" && (
          <button
            ...
            onClick={() => onCheckIn(stay.id, booking.id)}
          ...
          >
            ...
            {actionLoading === stay.id ? "Checking in..." : "Check In"}
          </button>
        )}
      </div>
    </div>
