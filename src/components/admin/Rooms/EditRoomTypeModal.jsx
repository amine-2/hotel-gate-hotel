...
export default function EditRoomTypeModal({
  ...
  useEffect(() => {
    ...
    setBeds(
      Array.isArray(roomType.beds) && roomType.beds.length > 0
    ...
    setAmenities(Array.isArray(roomType.amenities) ? roomType.amenities : []);

    setFreeCancellation(Boolean(roomType.free_cancellation));

    setDiscount(
      roomType.discount === null || roomType.discount === undefined
        ? ""
        : roomType.discount,
    );
  ...
  function handleDescriptionChange(language, value) {
    setDescription((prev) => ({
      ...prev,
      [language]: value,
    }));
  ...
  function removeBed(index) {
    setBeds((prev) => {
      ...
      return prev.filter((_, bedIndex) => bedIndex !== index);
    });
  }
  ...
  async function handleRemoveRoom(room) {
    ...
    const { error } = await removeRoomFromRoomType(
      hotelId,
      roomType.id,
      room.id,
    );
    ...
    setAssignedRooms((prev) => prev.filter((item) => item.id !== room.id));
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { data: updatedRoomType, error: updateError } =
        await updateRoomType(hotelId, roomType.id, {
          name: {
            en: name.en.trim(),
            fr: name.fr.trim(),
            ar: name.ar.trim(),
      ...
      const originalImages = Array.isArray(roomType.images)
        ? roomType.images
        : [];
      ...
      for (const imageUrl of removedImages) {
        const path = getRoomTypeImagePath(imageUrl);

        if (!path) {
          console.warn("Could not determine Storage path for image:", imageUrl);
          continue;
        }

        const { error: deleteError } = await deleteRoomTypeImage(path);
      ...
      for (const image of images) {
        ...
        const { data: uploadedImage, error: uploadError } =
          await uploadRoomTypeImage(hotelId, roomType.id, image.file);
        ...
        finalImageUrls.push(uploadedImage.url);
      }
      ...
      const { data: finalRoomType, error: imageUpdateError } =
        await updateRoomTypeImages(hotelId, roomType.id, finalImageUrls);
      ...
      console.error("Failed to update room type:", submitError);

      // Clean up newly uploaded files
      // if something failed afterward.
      for (const path of newlyUploadedPaths) {
        const { error: cleanupError } = await deleteRoomTypeImage(path);
      ...
      setError(submitError?.message || "Failed to update room type.");
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between border-b px-6 py-5 dark:border-zinc-700">
          <div>
          ...
          <button
            ...
            onClick={onClose}
            disabled={saving}
            className="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-900 disabled:opacity-50 dark:hover:bg-zinc-700 dark:hover:text-white"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="space-y-7 p-6">
          ...
          {error && (
            <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600 dark:border-red-800 dark:bg-red-800 dark:text-red-200">
              {error}
            </div>
          )}
          ...
          <section>
            <div className="mb-3">
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                The first image is the main image. Drag images to change their
                order.
              </p>
            </div>
                ...
                {images.map((image, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative aspect-4/3 overflow-hidden rounded-xl  bg-gray-100 dark:bg-gray-700 dark:border-zinc-700 ${
            ...
            <div className={images.length > 0 ? "mt-4" : ""}>
              <button
                ...
                disabled={saving}
                className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray-300 px-4 py-7 text-sm text-gray-500 transition hover:border-gray-400 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-600 dark:hover:bg-zinc-700 dark:hover:text-white dark:text-gray-400"
              >
                <ImagePlus size={20} />

                {images.length ? "Add more images" : "Upload room images"}
              </button>
          ...
          <section>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <TextField
                label="French"
                value={name.fr}
                onChange={(value) => handleNameChange("fr", value)}
              ...
              <TextField
                label="Arabic"
                value={name.ar}
                onChange={(value) => handleNameChange("ar", value)}
          ...
          <section>
            ...
            <div className="space-y-4">
              ...
              <TextAreaField
                ...
                onChange={(value) => handleDescriptionChange("fr", value)}
              ...
              <TextAreaField
                ...
                onChange={(value) => handleDescriptionChange("ar", value)}
          ...
          <section>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button
                ...
                onClick={addBed}
                disabled={saving}
                className="flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700  dark:hover:text-white"
              >
                <Plus size={16} />
                Add bed
              </button>
            ...
            <div className="space-y-3">
              {beds.map((bed, index) => (
                <div
                  key={index}
                  className="flex items-end gap-3 rounded-xl border border-gray-200 p-3 dark:border-gray-400"
                >
                  <div className="flex-1">
                    ...
                    <select
                      value={bed.type}
                      onChange={(event) =>
                        handleBedChange(index, "type", event.target.value)
                      }
                      disabled={saving}
                  ...
                  <div className="w-28">
                    ...
                    <input
                      ...
                      value={bed.quantity}
                      onChange={(event) =>
                        handleBedChange(index, "quantity", event.target.value)
                      }
                      disabled={saving}
                      className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-gray-400 dark:bg-zinc-800 dark:text-zinc-300"
                    />
                  ...
                  <button
                    ...
                    disabled={saving || beds.length === 1}
                    className="rounded-lg border border-red-200 p-2.5 text-red-500 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-red-400 dark:text-red-400 dark:hover:bg-red-800  "
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
          ...
          <section>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button
                ...
                disabled={saving}
                className="flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50  dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-700"
              >
                <Plus size={16} />
                Add Rooms
              </button>
            ...
            {assignedRooms.length === 0 ? (
              <div className="rounded-xl border border-dashed border-gray-300 px-4 py-8 text-center dark:border-gray-500 dark:bg-zinc-800 dark:text-zinc-300">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  No physical rooms are assigned to this room type.
                </p>
                ...
                <button
                  ...
                  onClick={() => setShowAddRooms(true)}
                  disabled={saving}
                  className="mt-3 text-sm font-medium text-gray-900 hover:underline disabled:opacity-50 dark:text-white"
                >
                  Add rooms
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {assignedRooms.map((room) => (
                  <div
                    key={room.id}
                    className="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300  "
                  >
                    <div>
                    ...
                    <button
                      ...
                      disabled={saving || removingRoomId === room.id}
                      className="rounded-md p-1.5 text-gray-400 transition hover:bg-white hover:text-red-600 disabled:opacity-40 dark:hover:bg-zinc-700 dark:hover:text-red-400"
                      title="Remove room"
                    >
                      <Trash2 size={15} />
                    </button>
            ...
            <div className="space-y-4">
              <label className="flex cursor-pointer items-center gap-3">
                <input
                  ...
                  onChange={(event) =>
                    setFreeCancellation(event.target.checked)
                  }
                  disabled={saving}
        ...
        <div className="flex justify-end gap-3 border-t px-6 py-4">
          <button
            ...
            onClick={onClose}
            disabled={saving}
            className="rounded-lg border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-700"
          >
            Cancel
          </button>
          ...
          <button
            ...
            disabled={saving}
            className="rounded-lg bg-gray-900 px-5 py-2.5 text-sm font-medium text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-orange-400 dark:hover:bg-orange-500"
          >
