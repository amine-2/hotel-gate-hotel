...
export default function AreaChart({
  ...
  if (xLabels.length > 20) {
    rotate = 60;
    labelFormatter = (val) => val.split(" ")[1] || val;
  } else if (xLabels.length > 10) {
    rotate = 30;
    labelFormatter = (val) => val.split(" ")[0] || val;

  }
   
  const theme = getChartTheme(isDark);
  const option = {
    title: { text: title, left: "left", textStyle: { color: theme.textColor } },
    ...
    tooltip: {
      ...
      formatter: (params) =>
        params
          .map(
            (p) =>
              `${p.marker} ${p.seriesName}<br/>${p.axisValue}: ${p.data}`
          )
          .join("<br/>"),
    },
    ...
    xAxis: {
      type: "category",
      boundaryGap: false, // 🔥 important for area chart
      data: xLabels,
      axisLabel: {
        interval: 0,
        rotate,
    ...
    series: series.map((s) => ({
      ...
      showSymbol: false, // cleaner look
      areaStyle: {
        opacity: 0.5, // area fill transparency
      },
      lineStyle: {
        width: 3,
      },
